<template>
    <div class="add-promo-popup add-news-popup">
        ...
        <form class="add-promo-popup-cont" @submit.prevent="submitForm($event, 'submit')" ref="newsForm"
            ...
            <div class="line-add-promo add-promo-popup-file">
                <label class="menuAddDishes-img">
                    ...
                    <div v-if="!data?.banner || data.banner?.length < 1"
                        class="ani imgArea popup-edit-vstk first-img-cont-news-popup">
                    ...
                    <div v-else class="ani imgArea popup-edit-vstk withIMG popup-edit-vstk-second"
                        :style="`background-image:url(${imgLinkHandler(data)})`">
                    </div>
                    <div v-if="hasImage || (data?.banner && data.banner?.length < 1)" class="edit-vstk-img-control">
                        <div class="edit-vstk-img-control-cont">
                            ...
                            <div class="mini-popup-control mini-popup-control-news-popup">
            ...
            <input class="line-add-promo" required placeholder="Новостной заголовок" :value="data?.title" type="text"
                name="newsName">
            <textarea placeholder="Содержание" required :value="data?.text" name="newsDescription"></textarea>
            <div class="form-new-news-group-button">
...
export default {
    ...
    computed: {
        imageClass() {
            if (this.hasImage || (this.data?.banner?.length > 0 && this.data?.banner[0]?.path !== null)) {
                return 'withIMG'
            } else {
                return ''
            }
    ...
    mounted() {
        ...
        const observer = new MutationObserver(() => {
            if (popupImgLabel.classList.contains("withIMG"))
                this.hasImage = true
        })
        observer.observe(popupImgLabel, { attributes: true, attributeFilter: ['class'] })


        const currentModal = document.querySelector(".add-news-popup");
        const observer2 = new MutationObserver((mutations) => {
            const popupImg = document.querySelector('.withIMG')
            if (popupImg)
                this.hasImage = true
            mutations.forEach(() => {
                if (!currentModal.classList.contains('active')) {
                    ...
                    this.hasImage = false
                    this.$store.dispatch('utils/setEditDish', {})

                }
            })
        })
        observer2.observe(currentModal, { attributes: true, attributeFilter: ['class'] })
    },
    methods: {
        changeFile(e) {
        ...
        controlPopupHandler() {
            const miniPopup = document.querySelector(".mini-popup-control-news-popup");
            if (miniPopup)
                miniPopup.classList.toggle("mini-popup-control-active")
        },
        deleteImgHandler() {
            if (this.data.banner && this.data.banner.length > 0) {
                this.data.banner = []
                this.hasImage = false
            }
            const contolPopup = document.querySelector(".mini-popup-control-news-popup");
            if (contolPopup && contolPopup.classList.contains('mini-popup-control-active'))
                contolPopup.classList.remove('mini-popup-control-active')
            const popupBack = document.querySelector('.first-img-cont-news-popup')
            if (popupBack) {
                if (popupBack.classList.contains("withIMG"))
                    popupBack.classList.remove("withIMG")
                popupBack.style.backgroundImage = 'none'
            }
        },
        submitForm(e, type = "submit") {
            ...
            if (!this.data?.title || this.newDataFlag) {
                let update = {
                    establishmentID: this.$route.params.id,
                    text: this.$refs.newsForm.newsDescription.value,
                    title: this.$refs.newsForm.newsName.value,
                    status: 'waiting',
                    banner: {},
                    city: this.cityEstb,
                    date: Date.now(),
                }
                if (this.$refs.newsForm?.img?.files[0]) {
                    formData.append('files[]', this.$refs.newsForm.img.files[0]);
                    let requestOptions = {
                        method: 'POST',
                        headers: {
                            'api-key': process.env.VUE_APP_COCKPIT_API_TOKEN
                    ...
                    fetch(process.env.VUE_APP_COCKPIT_ENDPOINT + 'api/assets/upload', requestOptions)
                        ...
                        .then(result => {
                            ...
                            fetch(process.env.VUE_APP_COCKPIT_ENDPOINT + 'api/content/item/news', requestOptionsPost)
                                .then(response => response.json())
                                .then(() => {
                                    if (type === "submit")
                                        this.$router.go()
                ...
                this.data.text = this.$refs.newsForm.newsDescription.value
                this.data.title = this.$refs.newsForm.newsName.value
                ...
                if (this.$refs.newsForm?.img?.files[0]) {
                    ...
                    fetch(process.env.VUE_APP_COCKPIT_ENDPOINT + 'api/assets/upload', requestOptions)
                        .then(response => response.json())
                        .then(result => {
                            this.data.banner = result.assets
                            let raw = JSON.stringify({
                                "data": this.data
                            })
                            requestOptionsPost.body = raw
                            fetch(process.env.VUE_APP_COCKPIT_ENDPOINT + `api/content/item/news?_id=${this.data._id}`, requestOptionsPost)
                                .then(response => response.json())
                                .then(() => {
                                    if (type === "submit")
                                        this.$router.go()
