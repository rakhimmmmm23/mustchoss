<template>
    <form method="post" class="novoe" enctype="multipart/form-data" @submit.prevent="handleSave">
        ...
        <div class="form__row form__row__2">
            <label class="form__input" :class="{ filled: isFilled(formData.name), error: error.name }"
                error="Введите название заведения">
                <input type="text" name="name" v-model="formData.name" required>
                <span class="label">Название заведения</span>
            </label>

            <div class="form__select" :class="{ error: error.type }" error="Выбирете тип заведения">
                <span class="label noselect ani" tabindex="1">Выбирете тип заведения</span>
        ...
        <div class="form__row form__row__2">
            <div class="form__select">
                <span class="label noselect ani" tabindex="1">Алматы</span>
                <div class="selectOptions ani">
        ...
        <div class="formMap form__row" id="ymap">
            <YandexMap :settings="ymap.settings" :coordinates="ymap.center" @click="onMapClick" :zoom="ymap.zoom"
                ...
                <YandexMarker :coordinates="ymap.coordinates" :marker-id="Date.now()" :icon="ymap.markerIcon">
                </YandexMarker>
            </YandexMap>
        </div>
...
export default {
    ...
    setup() {
        useHead({
            script: [
                {
                    src: '/static/js/establishments.js'
                }
            ],
        })
    },
    ...
    methods: {
        ...
        handleSave() {
            ...
            if (!isEmpty(this.formData.thumbs)) {
                update['thumbs'] = this.formData.thumbs
            }

            if (!isEmpty(this.formData.payments)) {
                update['payment'] = JSON.stringify(this.formData.payments)
            }

            if (!isEmpty(this.formData.kitchen)) {
            ...
            if (!isEmpty(name) && !isEmpty(type)) {
                fetch(process.env.VUE_APP_COCKPIT_ENDPOINT + 'api/content/item/establishments', requestOptions)
                    ...
                    .then(result => {
                        result
                        if (this.$store.state.auth.user.role === 'seller') {
                            this.$router.push('/biznes')
                        } else {
